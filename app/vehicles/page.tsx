        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f8f9fa', borderBottom: '1px solid #eee' }}>
              ...
              <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '12px', color: '#888', fontWeight: 600 }}>車種</th>
              ...
              <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '12px', color: '#888', fontWeight: 600 }}>車体価格</th>
          ...
          <tbody>
            {vehicles && vehicles.length > 0 ? vehicles.map((v, i) => (
              <tr key={v.id} style={{ borderBottom: i < vehicles.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
                <td style={{ padding: '10px 16px' }}>
                  {v.image_urls && v.image_urls.length > 0 ? (
                    <img src={v.image_urls[0]} alt={v.car_name}
                      style={{ width: '72px', height: '54px', objectFit: 'cover', borderRadius: '6px', border: '1px solid #eee' }} />
                  ) : (
                ...
                <td style={{ padding: '14px 16px', fontSize: '13px' }}>{v.body_price ? '¥' + v.body_price.toLocaleString() : '—'}</td>
                <td style={{ padding: '14px 16px' }}>
                  <span style={{
                    fontSize: '11px', padding: '3px 10px', borderRadius: '20px', fontWeight: 500,
                    background: statusColor[v.status]?.bg ?? '#f1f3f4',
                    color: statusColor[v.status]?.color ?? '#5f6368',
                  }}>{v.status}</span>
                </td>
              </tr>
            )) : (
